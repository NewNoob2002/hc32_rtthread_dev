...
static void keypad_read(lv_indev_drv_t *drv, lv_indev_data_t *data)
{
    ...
    // 获取按键动作
		key_action_t action = hal::FuncKey_GetAction();
    key_action_t action2 = PowerKey_GetAction();
    // 根据按键动作执行不同操作
    if (action != KEY_ACTION_NONE || action2 != KEY_ACTION_NONE) {
			
        if(action2 == KEY_ACTION_DOUBLE_CLICK)
				{
					last_key = LV_KEY_NEXT;
					key_pressed = true;
				}
				else if(action2 == KEY_ACTION_SINGLE_CLICK)
				{
					 last_key = LV_KEY_ENTER;
           key_pressed = true;
				}
				
				if(action == KEY_ACTION_SINGLE_CLICK)
				{
					 last_key = LV_KEY_DOWN;
           key_pressed = true;
				}
				else if(action == KEY_ACTION_DOUBLE_CLICK)
				{
					 last_key = LV_KEY_UP;
           key_pressed = true;
				}
...
void add_all_objs_to_group(lv_obj_t *parent, lv_group_t *group)
{
    ...
    for (uint32_t i = 0; i < child_cnt; i++) {
        ...
        // 检查是否是可聚焦控件类型
        if (
//            lv_obj_check_type(child, &lv_checkbox_class) ||   // Switch
