...
en_result_t irqn_aa_get(IRQn_Type &irqn, const char *name)
{
    if (irqn_aa_get_(&irqn) != Ok) {
        CORE_DEBUG_PRINTF("IRQn used up in get %s\n", name);
        CORE_ASSERT(false, "IRQn have been used up, no more to get\n", return ErrorInvalidParameter);
    }
...
en_result_t irqn_aa_get(IRQn_Type &irqn_num, const char *name)
{
	return irqn_aa_get_(&irqn_num);
}

en_result_t irqn_aa_resign(IRQn_Type &irqn_num, const char *name)
{
	return irqn_aa_resign_(&irqn_num);
}
#endif
