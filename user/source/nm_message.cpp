...
int NM_Message_Info_Pack(const char *pMsg, const char NM_PANEL_INFO_ID) {
  ...
  case NM_PANEL_INFO1_ID:
    memcpy(&msg[NM_PROTOCOL_HEADER_LEN + 0], HARDWARE_VERSION,
           strlen(HARDWARE_VERSION)); // HardWare_Version
    memcpy(&msg[NM_PROTOCOL_HEADER_LEN + 8], SOFTWARE_VERSION,
           strlen(SOFTWARE_VERSION)); // SoftWare_Version
    memcpy(&msg[NM_PROTOCOL_HEADER_LEN + 18], &batteryState.Percent, 2);
    memcpy(&msg[NM_PROTOCOL_HEADER_LEN + 20], &batteryState.Temp, 2);
    memcpy(&msg[NM_PROTOCOL_HEADER_LEN + 22], &batteryState.Voltage, 2);
    break;
  case NM_PANEL_INFO2_ID:
    ...
    msg[NM_PROTOCOL_HEADER_LEN + 4] = systemInfo.isCharging;    //充电电源接入
		systemInfo.record_op = 0;
		if(systemInfo.power_off_flag)Power_Monitor.PowerOFF_Ensure = true;
    break;
  case NM_PANEL_INFO3_ID:
  ...
  case NM_PANEL_INFO4_ID:
    ...
    memcpy(&msg[NM_PROTOCOL_HEADER_LEN + 4], &systemInfo.record_leftspace, 4);
    memcpy(&msg[NM_PROTOCOL_HEADER_LEN + 8], &systemInfo.record_name, 16);
    msg[NM_PROTOCOL_HEADER_LEN + 24] = systemInfo.record_type;
    msg[NM_PROTOCOL_HEADER_LEN + 25] = systemInfo.record_interval;
...
int NM_Message_SET_Pack(const char *pMsg, const char NM_PANEL_SET_ID) {
  ...
  case NM_PANEL_SET1_ID:
    if (systemInfo.mcu_operation_flag == 0) {
      systemInfo.work_mode = (work_mode_t)pMsg[NM_PROTOCOL_HEADER_LEN + 0];
      ...
      if (pMsg[NM_PROTOCOL_HEADER_LEN + 87])
        hal::Power_LinuxSetPowerOff(true);
    }
    break;
  case NM_PANEL_SET3_ID:
  ...
  case NM_PANEL_SET6_ID:
    if (systemInfo.mcu_operation_flag == 0) {
      ...
      memcpy(&systemInfo.record_leftspace, &pMsg[NM_PROTOCOL_HEADER_LEN + 4], 4);
      memcpy(&systemInfo.record_name, &pMsg[NM_PROTOCOL_HEADER_LEN + 8], 16);
      systemInfo.record_type = pMsg[NM_PROTOCOL_HEADER_LEN + 24];
      systemInfo.record_interval = pMsg[NM_PROTOCOL_HEADER_LEN + 25];
...
int NM_Message_Ctrl_Pack(const char *pMsg)
{
	...
	if(systemInfo.power_off_flag == 1)
	{
		systemInfo.sync_flag = true;
	}
	...
	msg[NM_PROTOCOL_PEP_OFFSET] = systemInfo.isCharging;
...
int NM_Message_Host_Pack(const char *pMsg)
{
	...
	if (!systemInfo.record_op)
	{
		if (state)
		{
			...
			hal::FuncLED_Switch(HIGH);
		}
		else
		{
			systemInfo.record_status = 0;
			hal::FuncLED_Switch(LOW);
		}
	}
...
int NM_Message_PowerOff_Pack(const char *pMsg)
{
  ...
  {
    systemInfo.power_off_flag = 0;
    //hal::Power_LinuxSetPowerOff(false);
  }
